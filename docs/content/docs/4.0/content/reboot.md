...
Tables are slightly adjusted to style `<caption>`s, collapse borders, and ensure consistent `text-align` throughout. Additional changes for borders, padding, and more come with [the `.table` class]({{ .Site.BaseURL }}/docs/{{ .Params.docs_version }}/content/tables/).
...
To merely toggle the visibility of an element, meaning its `display` is not modified and the element can still affect the flow of the document, use [the `.invisible` class]({{ .Site.BaseURL }}/docs/{{ .Params.docs_version }}/utilities/visibility/) instead.
