...
For a list of some of the browser bugs that Bootstrap has to grapple with, see our [Wall of browser bugs]({{ .Site.BaseURL }}/docs/{{ .Params.docs_version }}/browser-bugs/).
...
As of Safari v8.0, use of the fixed-width `.container` class can cause Safari to use an unusually small font size when printing. See [issue #14868]({{ .Params.repo }}/issues/14868) and [WebKit bug #138192](https://bugs.webkit.org/show_bug.cgi?id=138192) for more details. One potential workaround is the following CSS:
